...
class AlertPopup extends Popup {
  ...
  public function new(message:String, type:PopupType) {
    super(0.7, 0.4, BorderLayout.MIDDLE, false);
    ...
    var messageAndClose = function(message:String) {
      dispatchEvent(new DialogEvent(DialogEvent.MESSAGE, message));
      dispatchEvent(new DialogEvent(DialogEvent.CLOSED, TYPE));
      this.hide();
    };
    if (type == alert) {
